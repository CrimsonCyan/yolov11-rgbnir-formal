...
class ObjectAwareMultiScaleSmallPriorAuxConcat(ObjectAwareMultiScaleSmallPriorResidualReflectGateConcat):
    """P2 object-prior auxiliary supervision that leaves RGB/NIR features unchanged."""

    def forward(self, x):
        if len(x) != 2:
            raise ValueError(f"ObjectAwareMultiScaleSmallPriorAuxConcat expects 2 feature maps, got {len(x)}")

        rgb_feat, nir_feat = x
        if rgb_feat.shape[0] != nir_feat.shape[0] or rgb_feat.shape[2:] != nir_feat.shape[2:]:
            raise ValueError(
                "ObjectAwareMultiScaleSmallPriorAuxConcat requires matching batch/spatial shapes, "
                f"got {rgb_feat.shape} and {nir_feat.shape}"
            )
        ...
        if self.capture_object_gate and (self.training or not torch.is_grad_enabled()):
            ...
            self.last_object_gate = None
        self.last_residual_delta = None
        return torch.cat((rgb_feat, nir_feat), dim=self.d)


class ObjectAwareMultiScaleSmallPriorBoostConcat(ObjectAwareMultiScaleSmallPriorResidualReflectGateConcat):
    """P2 OA boost that preserves the original NIR path and adds a stronger object-prior residual."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            foreground_loss_weight=foreground_loss_weight,
            max_residual_scale=max_residual_scale,
            small_area_ref=small_area_ref,
            max_prior_weight=max_prior_weight,
        )
        if residual_init <= 0 or residual_init >= max_residual_scale:
            raise ValueError(
                "residual_init must be positive and smaller than max_residual_scale, "
                f"got {residual_init} and {max_residual_scale}"
            )
        self.residual_refine = Conv(nir_channels * 3, nir_channels, k=1, s=1)
        init_ratio = residual_init / max_residual_scale
        init_logit = math.log(init_ratio / (1.0 - init_ratio))
        with torch.no_grad():
            self.gate_scale.zero_()
            self.reflectance_scale.fill_(init_logit)

    def forward(self, x):
        if len(x) != 2:
            raise ValueError(f"ObjectAwareMultiScaleSmallPriorBoostConcat expects 2 feature maps, got {len(x)}")

        rgb_feat, nir_feat = x
        if rgb_feat.shape[0] != nir_feat.shape[0] or rgb_feat.shape[2:] != nir_feat.shape[2:]:
            raise ValueError(
                "ObjectAwareMultiScaleSmallPriorBoostConcat requires matching batch/spatial shapes, "
                f"got {rgb_feat.shape} and {nir_feat.shape}"
            )
...
class ObjectAwareP2HeadResidualRefineLite(nn.Module):
    ...
    def __init__(
        ...
        if min(p2_channels, rgb_channels, nir_channels) <= 0:
            raise ValueError(
                "ObjectAwareP2HeadResidualRefineLite expects positive channels, "
                f"got {p2_channels}, {rgb_channels}, {nir_channels}"
            )
        if small_area_ref <= 0:
            raise ValueError(f"small_area_ref must be positive, got {small_area_ref}")
        ...
        self.cue_reduce = Conv(cue_channels, cue_hidden, k=1, s=1)
        self.channel_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(cue_hidden, hidden, kernel_size=1, stride=1, padding=0, bias=True),
            nn.SiLU(inplace=True),
            nn.Conv2d(hidden, p2_channels, kernel_size=1, stride=1, padding=0, bias=True),
            nn.Sigmoid(),
        )
        self.object_prior = nn.Sequential(
            DWConv(cue_hidden, cue_hidden, k=3, s=1),
            nn.Conv2d(cue_hidden, 1, kernel_size=1, stride=1, padding=0, bias=True),
            nn.Sigmoid(),
        )
        self.refine = nn.Sequential(
            Conv(p2_channels * 3, cue_hidden, k=1, s=1),
            DWConv(cue_hidden, cue_hidden, k=3, s=1),
            Conv(cue_hidden, p2_channels, k=1, s=1, act=False),
        )
        self.gate_scale = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        init_ratio = float(residual_init) / float(max_residual_scale)
        init_logit = math.log(init_ratio / (1.0 - init_ratio))
    ...
    def forward(self, x):
        if len(x) != 3:
            raise ValueError(f"ObjectAwareP2HeadResidualRefineLite expects 3 feature maps, got {len(x)}")
        ...
        if (
            ...
            raise ValueError(
                "ObjectAwareP2HeadResidualRefineLite requires matching batch/spatial shapes, "
                f"got {p2_feat.shape}, {rgb_feat.shape}, {nir_feat.shape}"
            )
...
class ResidualQualityAwareFusion(nn.Module):
    """Residual quality-aware RGB/NIR fusion that preserves the original modality features."""

    def __init__(self, channels, dimension=1, reduction=4):
        super().__init__()
        if channels % 2 != 0:
            raise ValueError(f"ResidualQualityAwareFusion expects an even channel count, got {channels}")
        self.d = dimension
        self.single_channels = channels // 2
        hidden = max(self.single_channels // reduction, 1)
        quality_channels = self.single_channels * 3
    ...
    def forward(self, x):
        if len(x) != 2:
            raise ValueError(f"ResidualQualityAwareFusion expects 2 feature maps, got {len(x)}")

        rgb_feat, nir_feat = x
        if rgb_feat.shape != nir_feat.shape:
            raise ValueError(
                "ResidualQualityAwareFusion requires matching feature shapes, "
                f"got {rgb_feat.shape} and {nir_feat.shape}"
            )
        ...
        rgb_gain = rgb_weight * rgb_spatial
