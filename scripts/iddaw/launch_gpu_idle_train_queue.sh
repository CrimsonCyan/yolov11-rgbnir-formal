...
echo "[queue] imgsz=$IMGSZ batch=$BATCH cache=$DATA_CACHE optimizer=$OPTIMIZER lr0=$LR0 pretrained=$PRETRAINED mosaic=$MOSAIC close_mosaic=$CLOSE_MOSAIC"
...
for i in "${!MODES[@]}"; do
  ...
  while true; do
    ...
    WANDB_ENABLED="${WANDB_ENABLED:-1}" IDDAW_CLASS_SCHEMA=8cls_personmerge_traffic \
      ...
      PRETRAINED="$PRETRAINED" MOSAIC="$MOSAIC" CLOSE_MOSAIC="$CLOSE_MOSAIC" SMALL_CENTER_GAIN="$SMALL_CENTER_GAIN" SMALL_SCALE_GAIN="$SMALL_SCALE_GAIN" \
